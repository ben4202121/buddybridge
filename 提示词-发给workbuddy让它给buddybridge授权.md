...
**场景 A：通过 Obsidian 的 BuddyBridge 插件调用 CodeBuddy（最常见）**
...
我现在在 Obsidian 的 BuddyBridge 调用 CodeBuddy，现在没有写入权限，需要你帮我完成授权。
...
先问我两个问题：
1. 使用的是 **WorkBuddy 桌面版** 还是 **CodeBuddy CLI（包括通过 BuddyBridge 插件调用）**？
...
> ⚠️ **关键区别**：BuddyBridge 插件启动的是 CodeBuddy CLI，所以必须改 `~/.codebuddy/settings.json`，不是 `~/.workbuddy/settings.json`。这两个文件是不同的，改错文件会导致权限不生效。
...
- **如果重启后仍然被拒**，排查：
  1. 是否改对了文件（BuddyBridge 必须用 `~/.codebuddy/settings.json`）
  2. 文件是否有 BOM 头
...
1. **改错文件**：WorkBuddy 桌面版和 CodeBuddy CLI 的配置文件在不同目录（`.workbuddy` vs `.codebuddy`），BuddyBridge 启动的是 CodeBuddy，必须改 `.codebuddy` 下的文件。
...
4. **BuddyBridge 的 CLI 参数**：BuddyBridge 启动 CodeBuddy 时只传了 `--print` 和 `--session-id`，没有 `--permission-mode`，所以 `permMode` 始终是 `default`，必须依赖配置文件正确生效。
